...
> Vendored copy note: every variable in this table is `PLAYWRIGHT_MCP_*`, the configuration of the separate playwright-mcp server. In this repository at 74354ec the only environment variable the CLI itself reads is `PLAYWRIGHT_CLI_INSTALLATION_FOR_TEST` (`playwright-cli.js`); `PLAYWRIGHT_MCP_*` occurs nowhere outside this README. Configure the CLI with `open --config=<file>` and the flags above; treat this table as documentation of the MCP server until upstream says otherwise.
