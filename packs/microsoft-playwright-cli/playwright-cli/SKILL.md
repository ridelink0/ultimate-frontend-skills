...
## Beside ultimate-frontend-skills

When the ultimate-frontend-skills plugin is on the bench, its `look`, `quality`, `parity` and `verify` commands are the headless checks and this pack is what runs when a headless check cannot settle the question:

- `show --annotate` is the escalation. A render check has said the page passes and someone still thinks it looks wrong: open the page here, let them draw on it, and act on the notes.
- `set-reduced-motion reduce` is the same emulation `webdesign.mjs debug --motion reduce` uses. Reproduce a reduced-motion finding here before changing the CSS.
- `--raw snapshot` before and after an interaction, diffed, is the cheapest proof that a menu, accordion or dialog actually changed the tree. `look` reports interaction exceptions and layout shift under the pointer; it does not diff the tree.
- A screenshot taken here goes on the pull request with `gh pr comment --attach` (below); the PNGs `look` writes can be attached the same way.
- On Windows, every URL with more than one query parameter needs the `&` escaping in the section above, whichever tool is shelling it.
