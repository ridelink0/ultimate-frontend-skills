...
Run `gh --version` first: `gh` 2.98.0 (released 2026-08-20) has no `--attach` flag, so the commands below fail on it with an unknown-flag error. Upstream says `gh` 2.99+ uploads local images and videos with the repeatable `--attach` flag on `gh pr create`, `gh pr comment`, `gh pr edit`, `gh issue create`, `gh issue comment` and `gh issue edit`. PNG, JPEG, GIF, WebP, SVG, MP4, MOV and WebM are accepted, so `playwright-cli screenshot` and `video-start` output can be attached as is.
